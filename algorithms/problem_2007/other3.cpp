class Solution {
  public:
    // function for finding double using binary search
    ...
    vector<int> findOriginalArray(vector<int> &arr) {
        ...
        for (int i = 0; i < n; i++) {
            if (vis[i] == false) {
                ...
                // if double element is found, then mark single and double
                // element as found
                ...
                // if double element is not found in array, then return empty
                // array
                else if (idx == -1) {
                    return {};
                }
