...
public:
    ...
    void update_row(int N, int row, int delta) {
        for (auto& cell : cells) {
    ...
    void update_column(int N, int column, int delta) {
        for (auto& cell : cells) {
    ...
    void update_forward(int N, int row, int delta) {
        for (auto& cell : cells) {
    ...
    void update_backward(int N, int row, int delta) {
        for (auto& cell : cells) {
            if (cell.first.second + cell.first.first == row)
                cell.second.light += delta;
        }
    ...
    vector<int> gridIllumination(int N, vector<vector<int>>& lamps, vector<vector<int>>& queries)
    {
        ...
        for (auto& lamp: lamps) {
            ...
            ++rows[x], ++columns[y];
            ++forwards[y-x]; // the point that line y = y0 + (x-x0) intersects with the y-axis
            ++backwards[x+y]; // the point that line y = y0 - (x-x0) intersects with the y-axis
            auto it = cells.find(make_pair(x, y));
            if (it != cells.end())
                it->second.light -= 3;
            lamp_cells.insert(make_pair(x, y));
        }

        for (auto& row : rows)
            update_row(N, row.first, row.second);
        for (auto& column : columns)
            update_column(N, column.first, column.second);
        for (auto& forward: forwards)
            update_forward(N, forward.first, forward.second);
        for (auto& backward: backwards)
            update_backward(N, backward.first, backward.second);
        ...
        for (auto& q : queries) {
            ...
            if (--(it->second.count) == 0)
                cells.erase(it);
            rows.clear(), columns.clear(), forwards.clear(), backwards.clear();
