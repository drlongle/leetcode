...
Let F(i, j) be the number of minimum replacement required in s[0..i] if we replace s[i] with t[j],
G(i) be the number of minimum replacement required in s[0..i] if we don't replace s[i]
...
G(i) = min {
G(i - 1) if s[i] > s[i - 1],
F(i - 1, j) for any j where t[j] < s[i]
}

Many other solutions just attach G(i) either as F(i, n + 1) or F(i, 0), so there is only one function,
but it makes the formula much harder to understand.
 */
