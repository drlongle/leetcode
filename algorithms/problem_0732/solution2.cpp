...
A K-booking happens when K events have some non-empty intersection (ie., there is some time that is common
to all K events.)
...
void test_1()
{
    ...
    for (auto& slot: slots) {
        cout << slot.first << ", " << slot.second << " -> " << cal.book(slot.first, slot.second) << endl;
    }
