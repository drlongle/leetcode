...
The left parenthesis must be added to the left of '+' and the right parenthesis
...
public:
    string minimizeResult(string expression) {
        ...
        int n = stoi(expression.substr(0, index)) + stoi(expression.substr(index+1));
        ...
        for (int i = 0; i < index; i++) {
            for (int j = index+1; j < sz; j++) {
                ...
                if (i == 0) {
                    ...
                    x = stoi(expression.substr(0, i));
                }
                string p1 = expression.substr(i, index - i);
                string p2 = expression.substr(index +1 , j - index);
                int y = stoi(p1) + stoi(p2);
                int z;
                if (j == sz-1) {
                    z = 1;
                } else {
                    z = stoi(expression.substr(j+1, sz - j));
                }
                if (n > x*y*z) {
                    n = x*y*z;
                    string t = expression;
...
int main() {
    ...
    expression = "247+38";
    cout << sol.minimizeResult(expression) << endl;
