class Solution {
  public:
    ...
    int tree[8000]; // tree[i] holds the maximum height for its corresponding
                    // range
    ...
    int query(int t, int low, int high, int i, int j) {
        ...
        if (low == i && high == j)
            return tree[t];
        int mid = low + ((high - low) / 2);
        return max(query(2 * t, low, mid, i, min(mid, j)),
    ...
    vector<int> fallingSquares(vector<vector<int>> &positions) {
        ...
        for (auto it : s)
            mp[it] = compressed++;
        for (int i = 0; i < n; i++) {
            int start = positions[i][0], end = positions[i][1] + start - 1,
                h = positions[i][1];
