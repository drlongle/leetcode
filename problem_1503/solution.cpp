...
Note that the last moment when an ant was on the plank is t = 4 second, after that it falls imediately
out of the plank. (i.e. We can say that at t = 4.0000000001, there is no ants on the plank).
...
public:
    int getLastMoment(int n, vector<int>& left, vector<int>& right) {
        int res = 0;
        if (left.size()) {
            int pos = *max_element(begin(left), end(left));
            res = max(res, pos);
        }
        if (right.size()) {
            int pos = *min_element(begin(right), end(right));
            res = max(res, n - pos);
        }
        return res;
    }
