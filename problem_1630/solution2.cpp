...
public:
    vector<bool> checkArithmeticSubarrays(vector<int>& nums, vector<int>& l, vector<int>& r) {
        ...
        for (int i = 0; i < rsz; ++i) {
            ...
            auto prev = cnt.begin(), curr = next(prev);
            int diff = curr != cnt.end() ? curr->first - prev->first : 0;
            while (curr != cnt.end() && rs[i]) {
                rs[i] = (diff == curr->first - prev->first) && curr->second == 1 && prev->second == 1;
                ++prev, ++curr;
            }
        }
