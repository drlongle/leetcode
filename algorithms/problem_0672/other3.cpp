...
class Solution { // _Simulation
    static unordered_set<uint8_t>
    simulateStep(const unordered_set<uint8_t> &states, uint8_t mask) {
  ...
  public:
    int flipLights(int n, int presses) {
        ...
        // if presses > 4, then there must be p-4 presses that are cancelling
        // each other, and not
        // creating new states.
