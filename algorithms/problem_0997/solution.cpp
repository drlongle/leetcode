...
If the town judge exists and can be identified, return
the label of the town judge.  Otherwise, return -1.
...
public:
    int findJudge(int N, vector<vector<int>>& trust) {
        ...
        for (auto& t: trust) {
            nodes[t[0]].trust.emplace(t[1]);
            nodes[t[1]].trusted.emplace(t[0]);
        }

        for (int i = 1; i <= N; ++i) {
            if (nodes[i].trust.empty() && nodes[i].trusted.size() == N-1)
                return i;
        }
