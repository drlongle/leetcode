...
recursive function Successor(x, curOrder), which given a person x and the inheritance order so far, returns who should be the next person after x in the order of inheritance.
...
public:
    ThroneInheritance(string kingName): order({kingName}) {
        lookup[kingName] = order.begin();
    }

    list<string>::iterator skip(string name, list<string>::iterator& it) {
        ++it;
        while (it != order.end() && child[name].count(*it)) {
            it = skip(*it, it);
        }
        return it;
    }
    void birth(string parentName, string childName) {
        child[parentName].emplace(childName);
        auto it = lookup[parentName];
        it = skip(parentName, it);
        lookup[childName] = order.insert(it, childName);
    }
    ...
    unordered_map<string, list<string>::iterator> lookup;
...
void test_1() {
    ...
    copy(begin(order), end(order), ostream_iterator<string>(cout, ", "));
    cout << endl;
    t.death("bob"); // order: king > andy > matthew > bob > alex > asha > catherine
