  ...
  public:
    ...
    int robotSim(vector<int> &commands, vector<vector<int>> &obstacles) {
        ...
        for (auto &obstacle : obstacles)
            lookup[obstacle[0]].insert(obstacle[1]);
        for (auto command : commands) {
            if (command > 0) {
                switch (dir) {
                case Direction::North:
                    while (command > 0 && lookup[x].count(y + 1) == 0) {
                        ++y;
                        --command;
                ...
                case Direction::South:
                    ...
                    break;
                case Direction::East:
                    while (command > 0 && lookup[x + 1].count(y) == 0) {
                        ++x;
                        --command;
                        res = max(res, distance(x, y));
                    }
...
int main() {
    ...
    cout << "distance: " << sol.robotSim(commands, obstacles) << std::endl;
