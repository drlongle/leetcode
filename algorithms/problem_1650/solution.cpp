...
in a tree T is the lowest node that has both p and q as descendants (where we allow a node to
...
public:
    Node* lowestCommonAncestor(Node* p, Node * q) {
        ...
        reverse(begin(pq), end(pq));
        Node* res = nullptr;
        for (size_t i = 0; i < min(pp.size(), pq.size()); ++i) {
            if (pp[i] != pq[i])
                break;
            res = pp[i];
        }
