...
customers, all subsequent rotations are free in order to get all the customers down safely. Note that if
there are currently more than four customers waiting at the wheel, only four will board the gondola, and the
...
public:
    int minOperationsMaxProfit(vector<int>& customers, int boardingCost, int runningCost) {
        vector<int> wheels(4, 0);
        int round = 0, max_profit = 0, res = -1;
        int profit = 0, cnt = 0, index = 0, sz = customers.size();
        while (round < sz || cnt > 0) {
            if (round < sz) {
                cnt += customers[round];
            }
            int to_board = min(4, cnt);
            wheels[index] = to_board;
            cnt -= to_board;
            index = (index + 1) % 4;
            profit += to_board * boardingCost - runningCost;
            ++round;
