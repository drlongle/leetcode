/*
One trick to greatly speed up traversal of 'lakes' is with flood-fill (which seems oddly appropriate). Rather than push every single grid-point onto the min-heap, self-contained lakes, when reached, can be traversed in their entirety before pulling from the min-heap again. This works because lakes are effectively a large flat surface (meaning all points have the same height).
*/
