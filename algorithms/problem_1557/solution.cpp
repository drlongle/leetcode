...
public:
    ...
    vector<int> findSmallestSetOfVertices(int n, vector<vector<int>>& edges) {
        nodes.clear(), graph.clear(), visited.clear();
        ...
        for (auto& p: graph)
            dfs(p.first);
        vector<int> res(nodes.begin(), nodes.end());
        return res;
    }
};
...
int main() {
...

    {
        ...
        nodes = sol.findSmallestSetOfVertices(n, edges);
        copy(begin(nodes), end(nodes), ostream_iterator<int>(cout, ", "));
        cout << endl;
    }
