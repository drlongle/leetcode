...
class ParkingSystem {
public:
    ParkingSystem(int big, int medium, int small): slots(4, 0), limits(4) {
        limits[1] = big, limits[2] = medium, limits[3] = small;
    }

    bool addCar(int carType) {
        return ++slots[carType] <= limits[carType];
    }
    vector<int> slots, limits;
};
